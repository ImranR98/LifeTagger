...
module.exports.getFileInfo = (mediaDir, file) => {
    let existingTagString = this.taggedTestRegEx.exec(file)
    let existingTags = existingTagString ? existingTagString[0].slice(2, existingTagString.lastIndexOf(']')).split(',').map(tag => tag.trim()) : []
    let description = helpers.readFileDescription(`${mediaDir}/${file}`)
    return { file, existingTags, description }
}

/** Gets a list of all files in the provided path, along with each file's existing tags (if any) and current EXIF description */
module.exports.getFileList = (mediaDir, excludeExtensions) => {
    ...
    allTargets = allTargets.map(file => this.getFileInfo(mediaDir, file))
    return { targetFiles: allTargets, allTargetsCount: allTargets.length }
}

/** Gets an array of currently used tags for all target files in the provided path */
module.exports.getPrevUsedTags = (mediaDir) => {
    let tags = []
    this.getFileList(mediaDir).targetFiles.forEach(file => {
...
module.exports.tagFileEXIF = async (path, tags, description, deleteOriginal = true) => {
    let originalDescription = await helpers.readFileDescription(path).split('.')
