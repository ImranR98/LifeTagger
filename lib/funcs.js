...
module.exports.getFileList = async (mediaDir, excludeExtensions) => {
    ...
    for (let i = 0; i < allTargets.length; i++) {
        allTargets[i] = await this.getFileInfo(mediaDir, allTargets[i], false)
    }
...
module.exports.getPrevUsedTags = async (mediaDir) => {
    let tags = []
    await this.getFileList(mediaDir).targetFiles.forEach(file => {
